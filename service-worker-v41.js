// Service Worker v41 - Individual Habit Trends & Month View
const CACHE_VERSION = 'nous-v41-2026-02-08';
...
self.addEventListener('install', (event) => {
    ...
    event.waitUntil(
        (async () => {
            ...
            try {
                console.log('[SW v41] Caching critical files...');
                await cache.addAll(CRITICAL_FILES);
                console.log('[SW v41] ✓ Critical files cached');
            } catch (error) {
                console.error('[SW v41] ✗ Failed to cache critical files:', error);
            ...
            for (const file of OPTIONAL_FILES) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        console.log(`[SW v41] ✓ Cached optional file: ${file}`);
                    } else {
                        console.warn(`[SW v41] ⚠ Could not cache ${file}: HTTP ${response.status}`);
                    }
                } catch (error) {
                    console.warn(`[SW v41] ⚠ Failed to cache ${file}:`, error.message);
                    // Continue with installation even if optional files fail
                }
            }

            console.log('[SW v41] Installation complete, activating...');
            ...
            await Promise.all(
                cacheNames.map(cacheName => {
                    if (cacheName !== CACHE_NAME) {
                        console.log('[SW v41] Deleting old cache:', cacheName);
                        return caches.delete(cacheName);
                    }
                })
            );

            console.log('[SW v41] ✓ Activated and taking control');
        ...
        event.respondWith(
            (async () => {
                try {
                    ...
                    console.log('[SW v41] Network failed, trying cache');
                    ...
                    if (cachedResponse) {
                        console.log('[SW v41] ✓ Serving from cache offline');
                        return cachedResponse;
                    }

                    // Last resort: offline fallback page
                    console.log('[SW v41] ⚠ No cache available, showing offline page');
                    return new Response(
                        createOfflinePage(),
                        {
                            headers: {
        ...
        event.respondWith(
            (async () => {
                ...
                try {
                    ...
                    console.error('[SW v41] Failed to fetch:', url.pathname);
                    throw error;
                }
            })()
        );
...
self.addEventListener('message', (event) => {
    console.log('[SW v41] Received message:', event.data);
    ...
    if (event.data === 'CLEAR_CACHE') {
        event.waitUntil(
            caches.delete(CACHE_NAME).then(() => {
                console.log('[SW v41] Cache cleared');
            })
        );
    }
...
console.log('[SW v41] Service worker loaded and ready');
console.log('[SW v41] Cache version:', CACHE_VERSION);
