// Service Worker v35 - Active timer auto-selection on app load
const CACHE_VERSION = 'nous-v35-2026-01-24';
...
self.addEventListener('install', (event) => {
    ...
    event.waitUntil(
        (async () => {
            ...
            try {
                console.log('[SW v35] Caching critical files...');
                await cache.addAll(CRITICAL_FILES);
                console.log('[SW v35] ✓ Critical files cached');
            } catch (error) {
                console.error('[SW v35] ✗ Failed to cache critical files:', error);
            ...
            for (const file of OPTIONAL_FILES) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        console.log(`[SW v35] ✓ Cached optional file: ${file}`);
                    } else {
                        console.warn(`[SW v35] ⚠ Could not cache ${file}: HTTP ${response.status}`);
                    }
                } catch (error) {
                    console.warn(`[SW v35] ⚠ Failed to cache ${file}:`, error.message);
                    // Continue with installation even if optional files fail
                }
            }

            console.log('[SW v35] Installation complete, activating...');
...
self.addEventListener('activate', (event) => {
    ...
    event.waitUntil(
        (async () => {
            ...
            console.log('[SW v35] ✓ Activated and taking control');
        ...
        event.respondWith(
            (async () => {
                try {
                    ...
                    console.log('[SW v35] Network failed, trying cache');
                    ...
                    if (cachedResponse) {
                        console.log('[SW v35] ✓ Serving from cache offline');
                        return cachedResponse;
                    }

                    // Last resort: offline fallback page
                    console.log('[SW v35] ⚠ No cache available, showing offline page');
                    return new Response(
                        createOfflinePage(),
                        {
                            headers: {
        ...
        event.respondWith(
            (async () => {
                ...
                try {
                    ...
                    console.error('[SW v35] Failed to fetch:', url.pathname);
                    throw error;
                }
            })()
        );
...
self.addEventListener('message', (event) => {
    console.log('[SW v35] Received message:', event.data);
    ...
    if (event.data === 'CLEAR_CACHE') {
        event.waitUntil(
            caches.delete(CACHE_NAME).then(() => {
                console.log('[SW v35] Cache cleared');
            })
        );
    }
...
console.log('[SW v35] Service worker loaded and ready');
console.log('[SW v35] Cache version:', CACHE_VERSION);
