(function () {
    ...
    const GamificationTab = ({ user, db, isWidget = false }) => {
        ...
        useEffect(() => {
            ...
            const unsub2 = window.onSnapshot(window.collection(db, `/artifacts/${appId}/users/${userId}/rewards`), snap => {
                setRewards(snap.docs.map(d => ({ id: d.id, ...d.data() })));
            });
            const unsub3 = window.onSnapshot(window.doc(db, `/artifacts/${appId}/users/${userId}/gamification/wallet`), doc => {
        ...
        const toggleDate = async (habitId, dateStr) => {
            ...
            await window.updateDoc(window.doc(db, `/artifacts/${appId}/users/${userId}/habits/${habitId}`), {
                completionDates: newDates, streak: newDates.length
            });
        ...
        const addReward = async (e) => {
            ...
            await window.setDoc(window.doc(db, `/artifacts/${appId}/users/${userId}/rewards/${id}`), {
                id, title: newReward.title.trim(), cost: parseInt(newReward.cost) || 50
            });
        ...
        return React.createElement("div", { className: "container mx-auto px-4 py-6 max-w-4xl" },
            ...
            !showShop ? React.createElement(React.Fragment, null,
                // Week Nav
                React.createElement("div", { className: "flex items-center justify-between mb-6 p-3 bg-gradient-to-r from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 rounded-2xl" },
                    ...
                    React.createElement("div", { className: "text-center" },
                        ...
                        weekOffset !== 0 && React.createElement("button", {
                            onClick: () => setWeekOffset(0), className: "ml-3 text-sm text-blue-500 hover:underline lowercase"
                        }, "today")
                    ),
                    React.createElement("button", { onClick: () => setWeekOffset(w => w + 1), className: "p-2 hover:bg-white/50 rounded-lg" },
                ...
                showAddHabit && React.createElement("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" },
                    React.createElement("form", { onSubmit: addHabit, className: "bg-white dark:bg-gray-900 p-6 rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto" },
                        ...
                        React.createElement("input", {
                            type: "text", value: newHabit.title,
                            onChange: e => setNewHabit({ ...newHabit, title: e.target.value }),
                            placeholder: "e.g. Morning Run",
                            className: "w-full px-4 py-3 rounded-xl mb-4 border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white text-lg lowercase",
                            autoFocus: true
                        }),
                        // Colors
                        ...
                        React.createElement("div", { className: "flex gap-3 mb-4 flex-wrap" },
                            COLORS.map(c => React.createElement("button", {
                                ...
                                className: `w-10 h-10 rounded-full transition-transform ${newHabit.color === c ? 'scale-125 ring-2 ring-offset-2 ring-gray-300' : 'hover:scale-110'}`,
                        ...
                        React.createElement("div", { className: "grid grid-cols-6 gap-2 mb-4" },
                            Object.keys(ICONS).map(k => React.createElement("button", {
                                ...
                                className: `p-2 rounded-lg transition ${newHabit.icon === k ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600'}`
                        ...
                        React.createElement("div", { className: "flex gap-2 mb-5" },
                            ['easy', 'medium', 'hard'].map(d => {
                                ...
                                const colors = { easy: 'bg-green-500', medium: 'bg-orange-500', hard: 'bg-red-500' };
                                return React.createElement("button", {
                                    key: d, type: "button",
            ...
            showAddReward && React.createElement("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" },
                React.createElement("form", { onSubmit: addReward, className: "bg-white dark:bg-gray-900 p-6 rounded-2xl w-full max-w-sm shadow-2xl" },
                    React.createElement("div", { className: "flex justify-between items-center mb-4" },
                        ...
                        React.createElement("button", { type: "button", onClick: () => setShowAddReward(false), className: "p-2 text-gray-400" },
                    ...
                    React.createElement("input", {
                        ...
                        className: "w-full px-4 py-3 rounded-xl mb-4 border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white lowercase",
                        autoFocus: true
                    }),
                    React.createElement("input", {
