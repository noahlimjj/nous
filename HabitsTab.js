(function () {
    try {
        ...
        const HabitsPage = ({ user, db, isWidget = false, onToggleView, appId: propAppId, sharedTimers = [] }) => {
            ...
            useEffect(() => {
                if (window.OfflineTimerManager) {
                    ...
                    setTimers(prev => {
                        const next = { ...prev };
                        Object.values(offlineTimers).forEach(t => {
                            next[t.habitId] = {
        ...
        console.error("CRITICAL ERROR loading HabitsTab:", e);
        // Fallback to prevent white screen of death if possible, or just log
    }
})();
